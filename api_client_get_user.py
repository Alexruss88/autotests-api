...
from clients.users.private_users_client import get_private_users_client
from clients.users.public_users_client import get_public_users_client
from clients.users.users_schema import CreateUserRequestSchema  # или public_users_schema – смотрите по факту
from tools.fakers import get_random_email
...
authentication_user = AuthenticationUserSchema(
    email=create_user_request.email,  # если это Pydantic-модель, то через атрибут
    password=create_user_request.password
)

private_users_client = get_private_users_client(authentication_user)

private_users_client.get_user(create_user_response.user.id)  # или get_user(create_user_response.user.id) – как у вас
print('Get user data:', private_users_client.get_user(create_user_response.user.id))
